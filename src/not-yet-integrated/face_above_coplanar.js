...
import {range} from '/mesh/utils/range_array.js';
import {add, multiply, divide, tan, cross, angleBetween, unit} from '@oneisland/math';
...
pointsAboveCoplanar(height) {
  ...
  for (const vertex of this.vertices) {
    ...
    const faceNow = this.findAdjacentFaceFromEdgeIndex(indexOf(edgeNow));

    const faceBefore = this.findAdjacentFaceFromEdgeIndex(indexOf(edgeBefore));
    ...
    if (((cross(faceNow.normal, faceBefore.normal) > epsilon) && ((cross(face.normal, faceBefore.normal) > epsilon) && ((cross(faceNow.normal, face.normal) > epsilon))))) {
      ...
      point = this.plane.pointOfIntersectionWithPlanes(faceBefore.plane, faceNow.plane);
    }
    ...
    // so we return the point plus the height times the normal of the original face, so the point is directly above where it was by a distance of height.
      ...
      // If we have 3 faces that contain the point and are not parallel
        ...
        point = result[0].plane.pointOfIntersectionWithPlanes(result[1].plane, result[2].plane);
